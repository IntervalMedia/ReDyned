...
class ResultsViewController: UIViewController {
    ...
    private lazy var segmentedControl: UISegmentedControl = {
        var items = ["Info", "Symbols", "Strings", "Code", "Functions"]
        // Only show Hex if we have a file or cached data
        if output.fileSize > 0 || output.fileData != nil {
            items.append("Hex")
        }
    ...
    private lazy var stringsViewController: StringsViewController = {
        let vc = StringsViewController(strings: output.strings)
    ...
    private lazy var classDumpViewController: ClassDumpViewController? = {
        guard let classDumpHeader = output.classDumpHeader, !classDumpHeader.isEmpty else { return nil }
        return ClassDumpViewController(classDumpHeader: classDumpHeader)
    }()

    private lazy var typeReconstructionViewController: TypeReconstructionViewController? = {
        guard let typeResult = output.typeReconstructionAnalysis as? TypeReconstructionResultsObject else { return nil }
        return TypeReconstructionViewController(results: typeResult)
    }()
    ...
    private lazy var hexViewerViewController: HexViewerViewController? = {
        ...
        return HexViewerViewController(
            fileURL: fileURL,
            fileData: output.fileData as Data?,
            segments: segments,
            sections: sections,
            functions: functions,
            symbols: symbols
    ...
    private lazy var functionImportButton: UIBarButtonItem = {
        return UIBarButtonItem(
            image: UIImage(systemName: "arrow.down.doc"),
            style: .plain,
            target: self,
            action: #selector(showFunctionImportPicker)
        )
    }()
    ...
    @objc private func showMoreOptions() {
        ...
        let hasClassDump = (output.classDumpHeader?.isEmpty == false)
        let typeResult = output.typeReconstructionAnalysis as? TypeReconstructionResultsObject
        let hasTypeReconstruction = (typeResult?.types.isEmpty == false)

        let menuVC = AnalysisMenuViewController(
            hasObjCData: hasObjCData,
            hasCodeSignature: hasCodeSignature,
            hasClassDump: hasClassDump,
            hasTypeReconstruction: hasTypeReconstruction
        )
    ...
    @objc private func segmentChanged() {
        ...
        case 5:
            if let hexVC = hexViewerViewController {
                showViewController(hexVC)
            } else {
    ...
    private func updateFunctionImportButton() {
        if segmentedControl.selectedSegmentIndex == 4 {
            navigationItem.leftBarButtonItem = functionImportButton
        } else {
...
extension ResultsViewController: AnalysisMenuDelegate {
    func didSelectAnalysisType(_ type: AnalysisType) {
        ...
        case .classDump:
            if let classDumpVC = classDumpViewController {
                navigationController?.pushViewController(classDumpVC, animated: true)
            } else {
                showNoDataAvailable(type: "Class Dump")
            }
        case .typeReconstruction:
            if let typeVC = typeReconstructionViewController {
                navigationController?.pushViewController(typeVC, animated: true)
            } else {
                showNoDataAvailable(type: "Type Reconstruction")
            }
        case .imports:
...
class SymbolsViewController: UITableViewController {
    ...
    func filterSymbols(query: String) {
        if query.isEmpty {
            ...
            filteredSymbols = symbols.searchSymbols(query: query)
        }
...
class StringsViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
            let goToHexAction = UIAction(title: "Go to Address in Hex Viewer", image: UIImage(systemName: "number.square")) { [weak self] _ in
                self?.parentResultsVC?.showHexViewer(at: string.address)
            }
...
class FunctionsViewController: UITableViewController {
    ...
    func updateFunctions(_ updated: [FunctionModel]) {
        functions = updated.sortedByAddress()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "FunctionCell")
            ?? UITableViewCell(style: .subtitle, reuseIdentifier: "FunctionCell")
