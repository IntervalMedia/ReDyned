...
@objc class FilePickerViewController: UIViewController {
    ...
    private func showAbout() {
        let alert = UIAlertController(
            title: "ReDyne",
            message: "iOS reverse engineering suite for Mach-O binaries.\n\n\(Constants.App.versionLabel)\n\nDisassembly, control flow analysis, pseudocode, class dump, and type reconstruction.",
            preferredStyle: .alert
        )
...
extension FilePickerViewController: UITableViewDelegate {
            ...
            do {
                ...
                if isStale || url.path != path {
                    if let newBookmarkData = try? url.bookmarkData(
                        ...
                        updateRecentPath(from: path, to: url.path, bookmarkData: newBookmarkData)
                    }
                }
    ...
    private func updateRecentPath(from oldPath: String, to newPath: String, bookmarkData: Data) {
        guard oldPath != newPath else {
            UserDefaults.standard.saveFileBookmark(bookmarkData, for: oldPath)
            return
        }

        if let index = recentFiles.firstIndex(of: oldPath) {
            recentFiles[index] = newPath
            UserDefaults.standard.set(recentFiles, forKey: Constants.UserDefaultsKeys.recentFiles)
        }

        UserDefaults.standard.removeFileBookmark(for: oldPath)
        UserDefaults.standard.saveFileBookmark(bookmarkData, for: newPath)
        tableView.reloadData()
    }
