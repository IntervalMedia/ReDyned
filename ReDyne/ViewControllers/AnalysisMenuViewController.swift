...
enum AnalysisType: String, CaseIterable {
    ...
    var icon: String {
        ...
        case .typeReconstruction: return "square.stack.3d.down.forward"
    ...
    var description: String {
        ...
        case .classDump: return "Generated Objective-C headers"
        case .typeReconstruction: return "Inferred type definitions"
...
class AnalysisMenuViewController: UITableViewController {
    ...
    private let hasClassDump: Bool
    private let hasTypeReconstruction: Bool
    private let availableTypes: [AnalysisType]

    init(hasObjCData: Bool = false, hasCodeSignature: Bool = false, hasClassDump: Bool = false, hasTypeReconstruction: Bool = false) {
        self.hasObjCData = hasObjCData
        self.hasCodeSignature = hasCodeSignature
        self.hasClassDump = hasClassDump
        self.hasTypeReconstruction = hasTypeReconstruction
        ...
        for type in AnalysisType.allCases {
            ...
            case .classDump:
                if hasClassDump { types.append(type) }
            case .typeReconstruction:
                if hasTypeReconstruction { types.append(type) }
            case .signature:
