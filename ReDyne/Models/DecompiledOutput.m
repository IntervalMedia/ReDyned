...
- (NSData *)exportAsPDF {
    ...
    NSString *versionSummary = @"unknown";
    if (shortVersion.length > 0 && buildVersion.length > 0) {
        versionSummary = [NSString stringWithFormat:@"%@ (Build %@)", shortVersion, buildVersion];
    } else if (shortVersion.length > 0) {
        versionSummary = shortVersion;
    }
    NSString *footerText = [NSString stringWithFormat:@"Generated by ReDyne %@ - iOS Decompiler", versionSummary];
    [footerText drawAtPoint:CGPointMake(contentRect.origin.x, yPosition) withAttributes:footerAttrs];
