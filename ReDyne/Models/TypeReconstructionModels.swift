...
// MARK: - ObjC-Compatible Models

@objc class TypeReconstructedTypeObject: NSObject {
    ...
    init(name: String, category: String, size: Int, address: UInt64, confidence: Double) {
        self.name = name
        self.category = category
        self.size = size
...
@objc class TypeReconstructionResultsObject: NSObject {
    @objc let types: [TypeReconstructedTypeObject]

    init(types: [TypeReconstructedTypeObject]) {
        self.types = types
        super.init()
    }
}
