...
typedef struct {
    ...
    int64_t branch_offset;
     /* Use 64-bit masks to represent registers X0..X63 (we use bits 0..31 for X0..X31).
         This allows representing multiple registers written/read by an instruction. */
     uint64_t regs_read;
     uint64_t regs_written;
     /* NZCV flags written by instruction (bitmask: N=1, Z=2, C=4, V=8). */
...
/* Feature flags control decoder heuristics and verbosity. */
enum {
    DISASM_FLAG_PROLOGUE_EPILOGUE_HEURISTICS = (1u << 0),
    DISASM_FLAG_EXTENDED_DECODING            = (1u << 1),
    DISASM_FLAG_DETAILED_REGS                = (1u << 2),
};
