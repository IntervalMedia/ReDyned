...
- **Responsibilities**:
  ...
  - Refresh security-scoped bookmarks when recent file URLs change
...
- **Key Methods**:
  ...
  - **Caching Note**: After `BinaryParserService.parseBinary()` returns, `DecompileViewController` will attempt to read the binary file into memory and store it on the returned `DecompiledOutput.fileData`. This cached blob is used by UI viewers (for example the Hex Viewer) to avoid re-reading the original file from disk and to prevent "file not found" failures when the on-disk file is moved or removed.
...
- **Child Controllers**:
  ...
  - `ClassDumpViewController`: Generated Objective-C class headers
  - `TypeReconstructionViewController`: Reconstructed type summaries
  - **Hex Viewer Integration**: The segmented control can include a conditional "Hex" segment. The Hex segment is present only when `DecompiledOutput.fileSize > 0` or when a cached `fileData` blob is available. The `HexViewerViewController` prefers reading from `fileData` when present, otherwise falls back to reading the binary from disk.
...
**Properties**:
  ...
  - `classDumpHeader` (nullable `NSString`): generated Objective-C header content
  - `typeReconstructionAnalysis` (nullable `id`): reconstructed type summary payload
  - `fileData` (nullable `NSData`): optional in-memory cache of the original binary. Populated during `DecompileViewController.startDecompilation()` when possible so UI viewers can read the binary from memory instead of reloading from disk.
...
- **Large Data**: Lazy loading for instruction arrays (paginated display)
  - **Binary Cache**: The optional `fileData` cache improves viewer stability but increases memory usage. Consider adding a size cap (e.g., only cache files < X MB) or a user preference to enable/disable caching for very large binaries.
